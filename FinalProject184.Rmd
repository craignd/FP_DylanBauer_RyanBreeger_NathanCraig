...
**Visualizations**
Here we have a graph that shows a line graph for the price vs mileage of the BMW x3. As shown, the graph shows that there is a negative correlation between how many miles a car has when it is sold and the price that it is actually sold for. The second graph is the exact same measurements and axis, but with the data from the x5 instead of the x3. As seen, there is a negative correlation in both of the graphs
```{r}
...
**summary table for mileage on x3**
```{r}
...
```
**summary table for mileage on x5**
```{r}
# Create summary table for mileage
...
```
```
Shown above is a table with the merged data and the number of cars each. This is a good visualization because it shows a histogram full of both cars together, so viewers are able to interpret the prices easier and more generally



**Conclusions**
There are several takeaways from the graphs and data that can be shown throughout the project. The main takeaway is that there is a strong negative correlation between the number of miles on a used car and the price that it sold for. There was also a slight difference between the different types of cars and the quality that they kept over time. Although the x3 had a cheaper price at the beginning, the x5 had higher mean miles, meaning that it was more reliable over time. This makes sense because the x5 is considered a more luxurious vehicle, so the quality of parts and engine will be better. 
